...
class AgentConfig:
    ...
    # Identity
    service_name: str = field(default_factory=lambda: os.getenv("SERVICE_NAME", os.getenv("POD_NAME", "unknown-service")))
    ...
    # Operator API
    operator_api_url: str = field(default_factory=lambda: os.getenv("OPERATOR_API_URL", "http://localhost:8000").rstrip("/"))
    ...
    # Reporting
    report_interval_secs: int = field(default_factory=lambda: int(os.getenv("REPORT_INTERVAL_SECS", "30")))
    error_threshold: int = field(default_factory=lambda: int(os.getenv("ERROR_THRESHOLD", "5")))
    latency_threshold_ms: int = field(default_factory=lambda: int(os.getenv("LATENCY_THRESHOLD_MS", "1000")))
    ...
    max_buffer_reports: int = field(default_factory=lambda: int(os.getenv("MAX_BUFFER_REPORTS", "20")))
